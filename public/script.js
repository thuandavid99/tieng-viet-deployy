			...
			// ====================================
			let currentIndex = 0;
			let currentLessonSet = lessons; // Mặc định Tập 1. Đổi khi selectVolume(2)
			let currentReadingLesson = null;
			let currentStepIndex = 0;
			let readingStepRevealed = false;
			let currentLevel = 0;
			let playbackTimeout;
			...
			function selectVolume(vol) {
				stopAudio();
				currentLessonSet = (vol === 2) ? lessonsTap2 : lessons; // ← DÒNG MỚI: chọn đúng bộ data
			...
			function renderTocSlider() {
				...
				currentLessonSet.forEach((lesson, index) => {
					const slide = document.createElement('div');
					slide.className = 'lesson-slide';
					slide.dataset.index = index; // Lưu index vào data attribute
					
					if (lesson.type === "reading") {
						// Thẻ riêng cho bài đọc Tập 2 (không có char/svgKey)
						slide.innerHTML = `
							<div class="slide-card" onclick="playTingSound(); openLessonFromToc(${index})">
								<div class="slide-lesson-num">CHỦ ĐỀ ${lesson.chapter} · BÀI ${lesson.lessonInChapter}</div>
								<div class="slide-char long-text">${lesson.title}</div>
			...
			function openLessonFromToc(index) {
				stopAudio();
				currentIndex = index;
				const data = currentLessonSet[index];
				
				if (data.type === "reading") {
					openReadingLesson(data); // ← Bài Tập 2: mở màn hình đọc riêng
					return;
				}
			...
			// ====================================
			// MÀN HÌNH ĐỌC (TẬP 2) - chuỗi bước tuần tự, chạm để tiếp
			// ====================================
			function openReadingLesson(data) {
				stopAudio();
				document.getElementById('screen-start-merged').style.display = 'none';
				document.getElementById('screen-books').style.display = 'none';
				document.getElementById('screen-toc').style.display = 'none';
				document.getElementById('screen-lesson').style.display = 'none';
				document.getElementById('screen-reading').style.display = 'flex';

				currentReadingLesson = data;
				currentStepIndex = 0;
				readingStepRevealed = false;

				document.getElementById('reading-chapter-badge').innerText =
					`CHỦ ĐỀ ${data.chapter} · ${data.chapterName} · BÀI ${data.lessonInChapter}`;

				renderReadingStep();
			}

			function renderReadingStep() {
				const data = currentReadingLesson;
				const textEl = document.getElementById('reading-step-text');
				const iconEl = document.getElementById('reading-role-icon');
				const hintEl = document.getElementById('reading-hint');

				if (!data || !data.steps || currentStepIndex >= data.steps.length) {
					iconEl.innerText = '🎉';
					textEl.innerText = 'Hết bài rồi! Em giỏi lắm!';
					hintEl.innerText = '';
					speak("Hết bài rồi! Em giỏi lắm!", 1.0, () => {
						setTimeout(goBackToToc, 1000);
					});
					return;
				}
				...
				const roleIcons = { teacher: '👩‍🏫', narrator: '📖', student: '🧒' };

				if (step.type === 'narrate') {
					iconEl.innerText = roleIcons[step.role] || '📖';
					textEl.innerText = step.text;
					hintEl.innerText = '';
					speak(step.text);
				} else if (step.type === 'vocab') {
					iconEl.innerText = '📝';
					textEl.innerText = `${step.word}: ${step.meaning}`;
					hintEl.innerText = '';
					speak(`${step.word}, nghĩa là ${step.meaning}`);
				} else if (step.type === 'ask_aloud') {
					iconEl.innerText = '❓';
					textEl.innerText = step.prompt;
					hintEl.innerText = 'Chạm để nghe đáp án';
					speak(step.prompt);
				} else if (step.type === 'dictation') {
					iconEl.innerText = '✍️';
					textEl.innerText = step.text;
					hintEl.innerText = step.note || '';
					speak(step.text, 0.8);
				}
			}

			function replayReadingStep() {
				const data = currentReadingLesson;
				const step = data.steps[currentStepIndex];
				if (!step) return;

				if (step.type === 'ask_aloud' && !readingStepRevealed) {
					// Chạm lần đầu vào ô hỏi -> lộ đáp án, CHƯA sang bước tiếp theo
					readingStepRevealed = true;
					document.getElementById('reading-step-text').innerText = step.reveal;
					document.getElementById('reading-hint').innerText = 'Chạm ra ngoài để sang bước tiếp theo';
					speak(step.reveal);
				} else if (step.type === 'dictation') {
					speak(step.text, 0.7); // Nghe lại chậm hơn
				} else if (step.type === 'ask_aloud') {
					speak(step.reveal);
				} else {
					speak(step.text || `${step.word}, nghĩa là ${step.meaning}`);
				}
			}
			...
			function nextReadingStep() {
				const data = currentReadingLesson;
				const step = data.steps[currentStepIndex];
				// Nếu đang ở bước hỏi-đáp mà CHƯA lộ đáp án, chạm ra ngoài lần đầu = lộ đáp án (chưa mất bước)
				if (step && step.type === 'ask_aloud' && !readingStepRevealed) {
					replayReadingStep();
					return;
				}
				currentStepIndex++;
			...
			function goBackToToc() {
				...
				resetLevels();
				document.getElementById('screen-lesson').style.display = 'none';
				document.getElementById('screen-reading').style.display = 'none';
				document.getElementById('screen-books').style.display = 'none';
				document.getElementById('screen-toc').style.display = 'flex';
			...
			function speakLessonIntro(index) {
				console.log('🔊 Speaking lesson intro for index:', index, 'Lesson ID:', currentLessonSet[index]?.id);
				...
				speakTimeout = setTimeout(() => {
					...
					const lesson = currentLessonSet[index];
					
					let introText;
					if (lesson.type === "reading") {
						introText = `Chủ đề ${lesson.chapter}. Bài ${lesson.lessonInChapter}. ${lesson.title}`;
					} else {
						console.log('✅ Actually speaking lesson:', lesson.id, lesson.char);
						introText = `Bài ${lesson.id}. `;
			...
			function handleDoubleTap() {
				...
				const readingScreen = document.getElementById('screen-reading'); // ← MỚI
				...
				console.log('📱 Screen states:', {
					...
					reading: readingScreen.style.display,
				...
				if (lessonScreen.style.display === 'flex') {
					...
					goBackToToc();
				} else if (readingScreen.style.display === 'flex') {
					console.log('✅ Going back to TOC from reading screen');
					speak("Quay về mục lục", 1.0);
					document.getElementById('screen-reading').style.display = 'none';
					goBackToToc();
